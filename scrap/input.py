...
def get_all_input_tags(url=""):
    ...
    rows = soup.find_all('input')  # Extract and return first occurrence of tr
    for row in rows:
        print("####")
        print(row.attrs.get('id') or "none")
        print(row.attrs.get('name') or "none")
        if row.attrs.get('id') or row.attrs.get('name'):
            input_db.append(url)
        print("####")
...
with open(filepath) as fp:
   ...
   try:
       while line:
           ...
           print("Line {}: {}".format(cnt, url.strip()))
           try:
               get_all_input_tags(url=url)
               time.sleep(1)
           finally:
               pass
           cnt += 1
